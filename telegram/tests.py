...
class HabitApiTestCase(APITestCase):
    def setUp(self):
        ...
        self.habit1 = Habit.objects.create(
            ...
            estimated_time=datetime.timedelta(minutes=2),
            is_public=True
        )
        ...
        self.habit2 = Habit.objects.create(
            ...
            action='Пить воду',
            estimated_time=datetime.timedelta(minutes=1),
            is_public=False
        )
    ...
    def test_retrieve_own_habit(self):
        ...
        url = reverse('my-habit-retrieve-update-destroy', args=[self.habit1.pk])
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_retrieve_foreign_habit_forbidden(self):
        ...
        url = reverse('my-habit-retrieve-update-destroy', args=[self.habit2.pk])
        response = self.client.get(url)
    ...
    def test_update_own_habit(self):
        ...
        url = reverse('my-habit-retrieve-update-destroy', args=[self.habit1.pk])
        data = {'action': 'Делать утреннюю гимнастику', "place": "Дома", "time": "08:00:00", "estimated_time": "00:02:00"}
    ...
    def test_update_foreign_habit_forbidden(self):
        ...
        url = reverse('my-habit-retrieve-update-destroy', args=[self.habit2.pk])
        data = {'action': 'Попытка взлома', "place": "На работе", "time": "10:00:00", "estimated_time": "00:01:00"}
        response = self.client.patch(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
